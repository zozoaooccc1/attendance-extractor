...
export default function CaptureScreen() {
  ...
  // ── Check if guide should be shown before launching camera ──────────────
  ...
  // ── Continue with image after quality check ──────────────────────────────
  ...
  const launchCamera = async () => {
    if (Platform.OS === 'web') {
      Alert.alert(t.capture.title, t.capture.webOnly);
      router.back(); return;
    }
    ...
    try {
      const result = await ImagePicker.launchCameraAsync({
        mediaTypes: ['images'], allowsEditing: false, quality: 0.55, exif: false,
      });
        ...
        { text: t.capture.yesContinue, onPress: async () => {
          try {
            ...
            if (size < MIN_IMAGE_SIZE_BYTES) {
              Alert.alert(
                '⚠️ جودة الصورة منخفضة',
                'الصورة تبدو مظلمة أو ضبابية جداً. يُنصح بإعادة التصوير في مكان أكثر إضاءة.',
                [
                  { text: '📷 إعادة التصوير', style: 'cancel', onPress: () => launchCamera() },
                  { text: 'استخدام الصورة', onPress: () => continueWithImage(uri) },
                ]
              );
              return;
            }
  ...
  const handleConfirm = async () => {
    ...
    if (!isDbReady) {
      Alert.alert(t.error, 'قاعدة البيانات غير جاهزة، حاول مرة أخرى.');
      return;
    }
    setSaving(true);
    try {
      ...
      addRecord({
        ...
        confirmedTime: officialTime.displayTime, isManuallyEdited: false,
      ...
      if (isEntry) {
        ...
        const { isLate, minutesLate, graceLimitStr } = checkLateEntry(entryType, officialTime.displayTime, officialTime.time, resolvedShift);
        if (isLate) {
          showSuccessAndNavigate();
          setTimeout(() => Alert.alert('⚠️ تأخير في الدخول',
            `وقت الدخول: ${officialTime.displayTime}\nالحد: ${graceLimitStr}\nالتأخير: ${minutesLate} دقيقة`), 1500);
          return;
        }
      }
      showSuccessAndNavigate();
